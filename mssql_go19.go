...
// Date encodes parameters to Date SQL type.
type Date = civil.Date

// DateTime encodes parameters to DateTime2 SQL type.
type DateTime = civil.DateTime

// Time encodes parameters to Time SQL type.
...
func (s *Stmt) makeParamExtra(val driver.Value) (res param, err error) {
	...
	case Date:
		...
		buf := res.buffer
		days := val.DaysSince(Date{Year: 1970, Month: time.January, Day: 1})
	...
	case DateTime:
		...
		days := val.Date.DaysSince(Date{Year: 1970, Month: time.January, Day: 1})
		// number of days since Jan 1 1 UTC
		days = days + 1969*365 + 1969/4 - 1969/100 + 1969/400
		buf[5] = byte(days)
		buf[6] = byte(days >> 8)
